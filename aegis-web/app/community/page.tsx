import React from 'react';
import Image from 'next/image';
import Link from 'next/link';

// Reusable TeamMember Component
type SocialLink = {
    name: string;
    path: string;
  };

interface TeamMemberProps {
    imageSrc: string;
    name: string;
    position: string;
    socialLinks?: SocialLink[];
    widthClass?: string;
  }



const TeamMember = ({
    imageSrc,
    name,
    position,
    socialLinks = [
        { name: 'facebook', path: 'M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z' },
        { name: 'twitter', path: 'M8.29 20.251c7.547 0 11.675-6.253 11.675-11.675 0-.178 0-.355-.012-.53A8.348 8.348 0 0022 5.92a8.19 8.19 0 01-2.357.646 4.118 4.118 0 001.804-2.27 8.224 8.224 0 01-2.605.996 4.107 4.107 0 00-6.993 3.743 11.65 11.65 0 01-8.457-4.287 4.106 4.106 0 001.27 5.477A4.072 4.072 0 012.8 9.713v.052a4.105 4.105 0 003.292 4.022 4.095 4.095 0 01-1.853.07 4.108 4.108 0 003.834 2.85A8.233 8.233 0 012 18.407a11.616 11.616 0 006.29 1.84' },
        { name: 'instagram', path: 'M12.315 2c2.43 0 2.784.013 3.808.06 1.064.049 1.791.218 2.427.465a4.902 4.902 0 011.772 1.153 4.902 4.902 0 011.153 1.772c.247.636.416 1.363.465 2.427.048 1.067.06 1.407.06 4.123v.08c0 2.643-.012 2.987-.06 4.043-.049 1.064-.218 1.791-.465 2.427a4.902 4.902 0 01-1.153 1.772 4.902 4.902 0 01-1.772 1.153c-.636.247-1.363.416-2.427.465-1.067.048-1.407.06-4.123.06h-.08c-2.643 0-2.987-.012-4.043-.06-1.064-.049-1.791-.218-2.427-.465a4.902 4.902 0 01-1.772-1.153 4.902 4.902 0 01-1.153-1.772c-.247-.636-.416-1.363-.465-2.427-.047-1.024-.06-1.379-.06-3.808v-.63c0-2.43.013-2.784.06-3.808.049-1.064.218-1.791.465-2.427a4.902 4.902 0 011.153-1.772A4.902 4.902 0 015.45 2.525c.636-.247 1.363-.416 2.427-.465C8.901 2.013 9.256 2 11.685 2h.63zm-.081 1.802h-.468c-2.456 0-2.784.011-3.807.058-.975.045-1.504.207-1.857.344-.467.182-.8.398-1.15.748-.35.35-.566.683-.748 1.15-.137.353-.3.882-.344 1.857-.058 1.023-.057 1.351-.057 3.807v.468c0 2.456.011 2.784.058 3.807.045.975.207 1.504.344 1.857.182.466.399.8.748 1.15.35.35.683.566 1.15.748.353.137.882.3 1.857.344 1.054.048 1.37.058 4.041.058h.08c2.597 0 2.917-.01 3.96-.058.976-.045 1.505-.207 1.858-.344.466-.182.8-.398 1.15-.748.35-.35.566-.683.748-1.15.137-.353.3-.882.344-1.857.048-1.055.058-1.37.058-4.041v-.08c0-2.597-.01-2.917-.058-3.96-.045-.976-.207-1.505-.344-1.858a3.097 3.097 0 00-.748-1.15 3.098 3.098 0 00-1.15-.748c-.353-.137-.882-.3-1.857-.344-1.023-.047-1.351-.058-3.807-.058zM12 6.865a5.135 5.135 0 110 10.27 5.135 5.135 0 010-10.27zm0 1.802a3.333 3.333 0 100 6.666 3.333 3.333 0 000-6.666zm5.338-3.205a1.2 1.2 0 110 2.4 1.2 1.2 0 010-2.4z' }
    ],
    widthClass = 'w-full sm:w-3/4 lg:w-1/2 xl:w-1/4'
}: TeamMemberProps )=> {
    return (
        <div className={`${widthClass} relative group overflow-hidden rounded-lg`}>
            <Image
                src={imageSrc}
                height={600}
                width={600}
                alt={name}
                className="w-full h-auto object-cover transition-all duration-500 group-hover:scale-105 group-hover:brightness-50"
                layout="responsive"
            />
            <div className="absolute inset-0 flex flex-col bg-black bg-opacity-20 text-white px-4">
                <div className="transform transition-all duration-500 group-hover:-translate-y-4 mt-36">
                    <h2 className="text-md sm:text-lg font-bold transition-all duration-300 group-hover:text-lg sm:group-hover:text-xl">
                        {name}
                    </h2>
                    <p className="text-sm sm:text-sm transition-opacity duration-300">
                        {position}
                    </p>
                </div>
                {/* <div className="flex space-x-3 sm:space-x-4">
                    {socialLinks.map((icon, index) => (
                        <a
                            key={icon.name}
                            href="#"
                            className="text-white hover:text-gray-300 transition-all duration-300 transform translate-y-8 opacity-0 group-hover:opacity-100 group-hover:translate-y-0"
                            style={{ transitionDelay: `${index * 100 + 200}ms` }}
                        >
                            <svg className="w-6 h-6 sm:w-8 sm:h-8 hover:scale-125 transition-transform" fill="currentColor" viewBox="0 0 24 24">
                                <path fillRule="evenodd" d={icon.path} clipRule="evenodd" />
                            </svg>
                        </a>
                    ))}
                </div> */}
            </div>
        </div>
    );
};

const Community = () => {
    return (
        <div className='relative z-40 min-h-screen bg-[#F7F2F2]'>
            <div className="overflow-y-auto h-full pt-24 lg:pt-32">
                <div className="container mx-auto flex flex-col lg:flex-row px-4 sm:px-6 lg:px-8 py-8 font-sans text-gray-800 leading-relaxed">

                    {/* Navigation Sidebar */}
                    <div className="lg:w-1/4 mb-8 lg:mb-0">
                        <nav className="space-y-3 py-3 border-b lg:border-b-0 lg:border-r border-gray-200">
                            <Link href="/about" className="block text-gray-800 font-medium px-1 py-1 relative 
                                after:absolute after:bottom-0 after:left-0 after:w-0 after:h-0.5 after:bg-black 
                                after:transition-all after:duration-300 hover:after:w-full">
                                About us
                            </Link>
                            <Link href="/community" className="block text-maroon font-bold px-1 py-1 relative 
                                after:absolute after:bottom-0 after:left-0 after:w-0 after:h-0.5 after:bg-black 
                                after:transition-all after:duration-300 hover:after:w-full">
                                Our community
                            </Link>
                            <Link href="/team" className="block text-gray-800 font-medium px-1 py-1 relative 
                                after:absolute after:bottom-0 after:left-0 after:w-0 after:h-0.5 after:bg-black 
                                after:transition-all after:duration-300 hover:after:w-full">
                                Our team
                            </Link>
                        </nav>
                    </div>

                    {/* Main Content */}
                    <div className="lg:w-3/4 lg:pl-8">
                        {/* Community Intro Section */}
                        <div className='flex flex-col lg:flex-row gap-8 mb-8 lg:mb-12'>
                            <div className="lg:w-2/3">
                                <h1 className="text-2xl sm:text-3xl font-semibold mb-4 lg:mb-5 text-gray-900">Meet Our Community</h1>
                                <p className="mb-4">
                                    Our community members are actors in the peacebuilding and genocide prevention domain. The profiles include policymakers,
                                    academics, researchers, practitioners and even students in Rwanda, the region and beyond. Our aim is to connect you with
                                    a rich pool of different profiles who are experts in their respective fields.
                                </p>
                                <Link href="" className='inline-block bg-maroon text-white px-4 py-2 lg:p-3 rounded-md hover:bg-maroon-dark transition-colors'>
                                    Join Community
                                </Link>
                            </div>

                            {/* Community Image */}
                            <div className="lg:w-1/3">
                                <Image
                                    src='/community.png'
                                    alt='Community illustration'
                                    height={300}
                                    width={500}
                                    className="w-full h-auto object-cover"
                                    layout="responsive"
                                />
                            </div>
                        </div>

                        {/* Team Section */}
                        <div className="mt-4 lg:mt-16 space-y-8">
                            <h2 className="text-xl sm:text-2xl font-semibold">Research, Documentation & Policy Engagement team</h2>

                            {/* Using the reusable TeamMember component */}
                            <div className="flex flex-wrap gap-5">
                                <TeamMember
                                    imageSrc="/sandra.jpg"
                                    name="Sandra Shenge"
                                    position="Programmes Director"
                                    widthClass="w-full sm:w-3/4 lg:w-[23%]"
                                />


                                <TeamMember
                                    imageSrc="/aude.jpg"
                                    name="Aude Kamanzi Ibabaza"
                                    position="Research Documentation Manager"
                                    widthClass="w-full sm:w-3/4 lg:w-[23%]"
                                />

                                <TeamMember
                                    imageSrc="/landine.jpg"
                                    name="Landine Umuhire Marie Olga"
                                    position="Content Development Coordinator"
                                    widthClass="w-full sm:w-3/4 lg:w-[23%]"
                                />

                                <TeamMember
                                    imageSrc="/adelaide.jpg"
                                    name="Adélaïde Umugwaneza"
                                    position="Archivist"
                                    widthClass="w-full sm:w-3/4 lg:w-[23%]"
                                />
                            </div>

                            <div className="flex flex-wrap gap-5">
                                <TeamMember
                                    imageSrc="/claude.jpg"
                                    name="Jean Claude Rwigara"
                                    position="Transcriber"
                                    widthClass="w-full sm:w-3/4 lg:w-[23%]"
                                />


                                <TeamMember
                                    imageSrc="/jacques.jpg"
                                    name="Jacques Rubayiza"
                                    position="IT & Archive Officer"
                                    widthClass="w-full sm:w-3/4 lg:w-[23%]"
                                />

                                <TeamMember
                                    imageSrc="/prisca.jpg"
                                    name="Prisca Umwanankunda"
                                    position="Transcriber"
                                    widthClass="w-full sm:w-3/4 lg:w-[23%]"
                                />

                                     <TeamMember
                                    imageSrc="/michel.jpg"
                                    name="Michel Ndabaga"
                                    position="IT System Engineer"
                                    widthClass="w-full sm:w-3/4 lg:w-[23%]"
                                />


                            </div>

                            <div className="flex flex-wrap gap-5">
                           
                                <TeamMember
                                    imageSrc="/shema.jpg"
                                    name="Tristan Shema"
                                    position="Audio-Visual Content Development Officer"
                                    widthClass="w-full sm:w-3/4 lg:w-[23%]"
                                />
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    );
}

export default Community;